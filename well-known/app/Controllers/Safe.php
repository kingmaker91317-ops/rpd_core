...
public function index()
    ...
    public function index_post()
    {
        ...
        file_put_contents('debug_params.txt', "game: [" . ($game ?? 'null') . "] | user_key: [" . ($uKey ?? 'null') . "] | serial: [" . ($sDev ?? 'null') . "]\n", FILE_APPEND);
...
//=================================================
                            ...
                            // Get Seller Info from registrator / reseller / created_by field
                            ...
                            if (isset($findKey->registrator) && !empty($findKey->registrator)) {
                                ...
                                $data = [
                                    ...
                                    'data' => [
                                        ...
                                        'device'=> $max_dev,
                                        'rng' => $rngcnt,
                                        'ts' => (string)(strtotime($expiry) * 1000),
                                        'loop' => 'none',
                                        'urld' => 'https://rapijhbkhdcore.fun/Sock/POWER.zip'
                                    ],
                                ];
                            } else {
                                $data = [
