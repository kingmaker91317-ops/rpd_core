...

{
    ...
    protected $APP_SIGNATURE = "3836D249B410FA87E3D06E9A0902255749430626AABE5C5D5DA0E3591E6A1311";
...
<body>
    <div class="container">
        ...
        <p>Can't find a route for 'GlobalController'.</p>
    </div>
</body>
</html>
            <?php
        ...
        // ================= SERVER-SIDE VPN/PROXY DETECTION =================
        $hasVpnHeader = false;
        $proxyHeaders = [
            'HTTP_VIA',
            'VIA'
        ...
        // Fake 404 template response if VPN/Proxy header is found
        if ($hasVpnHeader) {
            header('Content-Type: text/html; charset=utf-8');
            http_response_code(404);
            ?>
...
<body>
    <div class="container">
        <h1>404 - File Not Found</h1>
        <p>Can't find a route for 'bsdk'.</p>
    </div>
</body>
</html>
            <?php
        ...
        if ($isMT) {
            ...
            if ($uKey === "GuestKey") {
                $findKey = (object)[
                    ...
                    'status' => 1,
                    'duration' => 99999,
                    'expired_date' => date("Y-m-d H:i:s", strtotime("+365 days")),
                    'devices' => $sDev,
                    'max_devices' => 99999,
                    'registrator' => 'System'
                ];
            ...
            if ($findKey) {
                if ($findKey->status != 1) {
                    ...
                    if (isset($responseData['status']) && $responseData['status']) {
                        ...
                        if ($devicesAdd !== false) {
                            if (is_array($devicesAdd) && $uKey !== "GuestKey") {
                                $model->update($id_keys, $devicesAdd);
                            }

                            // ================= DECRYPT EXISTING LOADER AND ENCRYPT WITH SESSION KEY =================
                            $encLoader = @file_get_contents(FCPATH . "golballlll/kernel.kmods");
                            $base64Load = "";
                            if ($encLoader) {
                                $aesKey = "A5D8F2E7B3C6A1D9E0F4D2C3B5E6F7A8";
                                $decLoader = openssl_decrypt($encLoader, 'aes-256-ecb', $aesKey, OPENSSL_RAW_DATA);
                                if ($decLoader !== false) {
                                    // Encrypt loader using raw XXTEA with sessionKey, then base64 encode
                                    $rawEncLoader = $this->xxtea_encrypt_raw($decLoader, $sessionKey);
                                    $base64Load = base64_encode($rawEncLoader);
                                } else {
                                    writeLog("Failed to decrypt PUBG.kmods using AES-256-ECB");
                                }
                            }
